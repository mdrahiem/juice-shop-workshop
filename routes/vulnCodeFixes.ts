...
export const checkCorrectFix = () => async (req: Request<Record<string, unknown>, Record<string, unknown>, VerdictRequestBody>, res: Response, next: NextFunction) => {
  ...
  if (fixData.fixes.length === 0) {
    ...
    let explanation
    if (fs.existsSync('./data/static/codefixes/' + path.basename(key) + '.info.yml')) {
      const codingChallengeInfos = yaml.load(fs.readFileSync('./data/static/codefixes/path.js', 'utf8'))
